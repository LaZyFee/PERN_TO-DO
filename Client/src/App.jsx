...
function App() {
  ...
  const onSubmitForm = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/todos/create-todo`,
        {
  ...
  const saveEdit = async (id) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/todos/update-todo/${id}`,
        {
  ...
  const deleteTodo = async (id) => {
    try {
      setError(null);
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/todos/delete-todo/${id}`
      );
  ...
  const toggleCompleted = async (id) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/todos/update-todo-status/${id}`,
      ...
      setTodos(
        todos.map((todo) =>
          todo.todo_id === id ? { ...todo, completed: updatedCompleted } : todo
        )
      );
    } catch (err) {
      console.error(err.message);
